...
class chk4data(threading.Thread):  ## Side thread to interogate SQL server for changed data

    def __init__(self):
        self.host="http://caver456.pythonanywhere.com"
        self.columns=["ID","Name","Agency","Resource","TimeIn","TimeOut","Total","InEpoch","OutEpoch","TotalSec","CellNum","Status","Synced"]
        threading.Thread.__init__(self)
        print("chk4data init")
        self.m = 0
        self.pntrx = 0
        self.prev_data = {}
        self.prev_num_entries = 0

    def run(self):    # is run a special name?

        while True:
            
        ## get sql data
          r=requests.get(url=self.host+"/api/v1/events"+self.pntrx.eventID)
        ...
        #  order to store in the internal list of lists
          if (r.status_code != 200):
              print("Error: Cloud connection failure")
              return
          else:  
            j = r.json() # r.json() returns a list of dictionaries
            jlen = len(j)
        ## compare to previous
            if (jlen != self.prev_num_entries or j != self.prev_data):
              self.pntrx.RMTchange = 1      # when 1 run updates in FrmtInProcess
              print("Changed data %i:%i"%(jlen,self.prev_num_entries))
              #C self.TApntr.rmtInProcess()
              self.prev_data = j
              self.prev_num_entries = jlen
            else:   
              print("No data change")

            self.m = self.m + 1
            print("In chk4data %i"%self.m)
...
class PaintTable(QtWidgets.QTableWidget):
            ...
            if (self.ccm >= 0 and self.rrm > 0):    ## on grided table
              ...
              if (self.fnd_team != -1):   ## pointing to a TEAM
                ...
                if (self.rrm > self.pntrtab.Nrows-4):
                  rx = (self.rrm-3)*25  #A
                self.pntrtab.tableWidget2.move((self.ccm+1)*145, rx)  #A
                ...
                rx = (self.rrm+1)*25  #A
                if (self.rrm > self.pntrtab.Nrows-8): #A
                  rx = (self.rrm-6)*25 #A
                cx = (self.ccm+1)*145
                if (self.ccm > self.pntrtab.Nsets-3):
                  cx = (self.ccm-2)*145+80
                self.pntrtab.tableWidget5.move(cx, rx)  #A
              ...
              self.pntrtab.tableWidget4.move(500,500)  #A # near center
    ...
    def paintEvent(self, event):
        ...
        self.scl = 2   ## this routine does not see the other self.scl ??
        ...
        painter.setPen(QtGui.QPen(QtCore.Qt.blue, 10*self.scl))  #A
        qfont = QFont('Times', 16, QFont.Bold)
        #A qfont = QFont('Times', 24, QFont.Bold)
        painter.setFont(qfont)
        painter.drawText(450*self.scl,0,600*self.scl,50*self.scl,QtCore.Qt.AlignLeft, "A s s i g n e d")
        painter.drawText(1150*self.scl,0,600*self.scl,50*self.scl,QtCore.Qt.AlignLeft, "U n A s s i g n e d")
        #A painter.drawText(500,-15,1200,100,QtCore.Qt.AlignLeft, "A s s i g n e d")
        #A painter.drawText(2100,-15,1200,100,QtCore.Qt.AlignLeft, "U n A s s i g n e d")
        line = QtCore.QLineF(750*self.scl+120, 0*self.scl, 750*self.scl+120, 760.0*self.scl)  ## divider btwn assigned / unassigned
        #A line = QtCore.QLineF(1620, 0, 1620, 1600.0)  ## divider btwn assigned / unassigned
        painter.drawLine(line)

        painter.setPen(QtGui.QPen(QtCore.Qt.red, 2*self.scl)) #A  # line width for rounded rectangles
        
        for ib in range(self.inxt):
          painter.drawRoundedRect(self.coordxp[ib]+1,self.coordyp[ib]+3,134*self.scl,22*self.scl,10*self.scl,10*self.scl)
          #Apainter.drawRoundedRect(self.coordxp[ib]+1,self.coordyp[ib]+3,268,44,20,20)
        painter.setPen(QtCore.Qt.blue)
        for ib in range(self.inxd):
          if (self.type[ib] == 1):  ## output MEDICAL blue dot
            painter.setBrush(QtCore.Qt.blue)
            center = QtCore.QPoint(self.coordxd[ib]+133*self.scl,self.coordyd[ib]+12*self.scl)  #A
          elif (self.type[ib] == 2):  ## output LEADER red dot
            painter.setBrush(QtCore.Qt.red)
            center = QtCore.QPoint(self.coordxd[ib]+123*self.scl,self.coordyd[ib]+12*self.scl)  #A            
          painter.drawEllipse(center,5*self.scl,5*self.scl)  #A
...
class Ui_MainWindow(object):     #QtWidgets.QTableWidget
    ec = ctypes.windll.shcore.SetProcessDpiAwareness(1)
    sw = GetSystemMetrics(0)
    sh = GetSystemMetrics(1)
    scl = sw/1500   # assume height is also scaled by the same amount
    print("Win: %i:%i"%(sw,sh))
    Hmain = sh-100 #1000   #A 1900  # in pixels
    Wmain = sw #1600  #A 3000  # in pixels
    Nrows = 30   
    Nsets = 11
    ...
    def setupUi(self, MainWindow):
        ...
        Nchars = 10*self.scl #A 20
        Hcell = 25*self.scl  #A 50
        Ncols = self.Nsets       
        ...
        self.tableWidget2.setGeometry(QtCore.QRect(200*self.scl, 260*self.scl, 200*self.scl, 125*self.scl))  #A
        ...
        self.tableWidget5.setGeometry(QtCore.QRect(200*self.scl, 260*self.scl, 200*self.scl, 250*self.scl))  #A
        ...
        ##  activated when displaying choices for Event to connect to when in Remote mode
        self.tableWidget6 = PaintTable(self.centralwidget)  ## secondary small table above primary table
        self.tableWidget6.setGeometry(QtCore.QRect(200*self.scl, 260*self.scl, 380*self.scl, 185*self.scl))  #A 

        self.tableWidget6.setRowCount(6)
        self.tableWidget6.setColumnCount(4)
        self.tableWidget6.setColumnWidth(0,50)  ## more room for resources
        self.tableWidget6.setColumnWidth(1,400)
        self.tableWidget6.setColumnWidth(2,200)
        self.tableWidget6.setColumnWidth(3,30)
        self.tableWidget6.setObjectName("tableWidget6")
        items = ("ID", "Description", "EventStart", "Check", "         Ok")
        for i in range(0,5):
          item = QtWidgets.QTableWidgetItem(items[i])
          item.setFlags(Qt.ItemIsEnabled)      # protected
          if (i < 4): self.tableWidget6.setItem(0,i,item)
          else:       self.tableWidget6.setItem(5,1,item)
        self.tableWidget6.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        self.tableWidget6.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        self.tableWidget6.horizontalHeader().hide()
        self.tableWidget6.verticalHeader().hide()        
        self.tableWidget6.hide()
        #  from RMB in Groups column
        self.tableWidget3 = PaintTable(self.centralwidget)  ## secondary small table above primary table
        self.tableWidget3.setGeometry(QtCore.QRect(200*self.scl, 260*self.scl, 100*self.scl, 170*self.scl))  #A
        ...
        self.tableWidget4.setGeometry(QtCore.QRect(200*self.scl, 260*self.scl, 200*self.scl, 180*self.scl))  #A
        ...
        self.tableWidget.pntrtab = self  ## set pntrtab value
        self.pushButton.setGeometry(QtCore.QRect(225*self.scl, 800*self.scl, 75*self.scl, 23*self.scl))  #A
        self.pushButton.setObjectName("pushButton")
        self.pushButton_2 = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_2.setGeometry(QtCore.QRect(325*self.scl, 800*self.scl, 75*self.scl, 23*self.scl))  #A
        self.pushButton_2.setObjectName("pushButton_2")
        self.pushButton_undo = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_undo.setGeometry(QtCore.QRect(25*self.scl, 800*self.scl, 75*self.scl, 23*self.scl)) #A
        self.pushButton_undo.setObjectName("pushButton_undo")
        self.pushButton_readMem = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_readMem.setGeometry(QtCore.QRect(125*self.scl, 800*self.scl, 75*self.scl, 23*self.scl))  #A
        self.pushButton_readMem.setObjectName("pushButton_readMem")
        self.pushButton_teams = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_teams.setGeometry(QtCore.QRect(610*self.scl, 790*self.scl, 75*self.scl,23*self.scl))  #A
        self.pushButton_teams.setObjectName("pushButton_teams")
        self.pushButton_prntScrn = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_prntScrn.setGeometry(QtCore.QRect(610*self.scl, 765*self.scl, 75*self.scl,23*self.scl))  #A
        self.pushButton_prntScrn.setObjectName("pushButton_prntScrn")
        self.pushButton_prnt104 = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_prnt104.setGeometry(QtCore.QRect(610*self.scl, 815*self.scl, 75*self.scl,23*self.scl))  #A
        self.pushButton_prnt104.setObjectName("pushButton_prnt104")
        
        self.num7 = QtWidgets.QPushButton(self.centralwidget)
        self.num7.setGeometry(QtCore.QRect(475*self.scl, 765*self.scl, 20*self.scl, 23*self.scl))  #A
        self.num7.setObjectName("num7")
        self.num8 = QtWidgets.QPushButton(self.centralwidget)
        self.num8.setGeometry(QtCore.QRect(500*self.scl, 765*self.scl, 20*self.scl, 23*self.scl))
        self.num8.setObjectName("num8")
        self.num9 = QtWidgets.QPushButton(self.centralwidget)
        self.num9.setGeometry(QtCore.QRect(525*self.scl, 765*self.scl, 20*self.scl, 23*self.scl))
        self.num9.setObjectName("num9")
        self.num4 = QtWidgets.QPushButton(self.centralwidget)
        self.num4.setGeometry(QtCore.QRect(475*self.scl, 790*self.scl, 20*self.scl, 23*self.scl))
        self.num4.setObjectName("num4")
        self.num5 = QtWidgets.QPushButton(self.centralwidget)
        self.num5.setGeometry(QtCore.QRect(500*self.scl, 790*self.scl, 20*self.scl, 23*self.scl))
        self.num5.setObjectName("num5")
        self.num6 = QtWidgets.QPushButton(self.centralwidget)
        self.num6.setGeometry(QtCore.QRect(525*self.scl, 790*self.scl, 20*self.scl, 23*self.scl))
        self.num6.setObjectName("num6")
        self.num1 = QtWidgets.QPushButton(self.centralwidget)
        self.num1.setGeometry(QtCore.QRect(475*self.scl, 815*self.scl, 20*self.scl, 23*self.scl))
        self.num1.setObjectName("num1")
        self.num2 = QtWidgets.QPushButton(self.centralwidget)
        self.num2.setGeometry(QtCore.QRect(500*self.scl, 815*self.scl, 20*self.scl, 23*self.scl))
        self.num2.setObjectName("num2")
        self.num3 = QtWidgets.QPushButton(self.centralwidget)
        self.num3.setGeometry(QtCore.QRect(525*self.scl, 815*self.scl, 20*self.scl, 23*self.scl))
        self.num3.setObjectName("num3")
        self.num0 = QtWidgets.QPushButton(self.centralwidget)
        self.num0.setGeometry(QtCore.QRect(475*self.scl, 840*self.scl, 20*self.scl, 23*self.scl))
        self.num0.setObjectName("num0")
        self.numR = QtWidgets.QPushButton(self.centralwidget)
        self.numR.setGeometry(QtCore.QRect(500*self.scl, 840*self.scl, 20*self.scl, 23*self.scl))
        self.numR.setObjectName("numR")
        self.numS = QtWidgets.QPushButton(self.centralwidget)
        self.numS.setGeometry(QtCore.QRect(525*self.scl, 840*self.scl, 20*self.scl, 23*self.scl))
        self.numS.setObjectName("numS")
        self.numD = QtWidgets.QPushButton(self.centralwidget)
        self.numD.setGeometry(QtCore.QRect(550*self.scl, 765*self.scl, 20*self.scl, 23*self.scl))  #A
        self.numD.setObjectName("numD")
        self.numE = QtWidgets.QPushButton(self.centralwidget)
        self.numE.setGeometry(QtCore.QRect(550*self.scl, 790*self.scl, 20*self.scl, 23*self.scl))
        self.numE.setObjectName("numE")
        self.numN = QtWidgets.QPushButton(self.centralwidget)
        self.numN.setGeometry(QtCore.QRect(550*self.scl, 815*self.scl, 20*self.scl, 23*self.scl))
        self.numN.setObjectName("numN")
        self.numT = QtWidgets.QPushButton(self.centralwidget)
        self.numT.setGeometry(QtCore.QRect(550*self.scl, 840*self.scl, 20*self.scl, 23*self.scl))
        self.numT.setObjectName("numT")
        self.numB = QtWidgets.QPushButton(self.centralwidget)
        self.numB.setGeometry(QtCore.QRect(575*self.scl, 765*self.scl, 20*self.scl, 23*self.scl))
        self.numB.setObjectName("numB")        
        self.numALP = QtWidgets.QPushButton(self.centralwidget)
        self.numALP.setGeometry(QtCore.QRect(575*self.scl, 790*self.scl, 20*self.scl, 23*self.scl)) #A
        ...
        self.dateTime.move(710*self.scl, 815*self.scl)  #A
        self.dateTime.resize(250*self.scl,23*self.scl)  #A
        self.dateTime.setFont(fntField)
        self.dateTime.setText("Date and Time")
        self.infox = QtWidgets.QLineEdit(self.centralwidget)
        self.infox.move(975*self.scl, 815*self.scl)  #A
        self.infox.resize(500*self.scl,23*self.scl)  #A
        ...
        self.sarID.move(420*self.scl, 825*self.scl)  #A
        self.sarID.resize(50*self.scl,20*self.scl)  #A
        self.sarID.setFont(fntSmall)
        self.sarID.setText("")
        self.sarIDlab = QtWidgets.QLabel(self.centralwidget)
        self.sarIDlab.move(420*self.scl, 800*self.scl)  #A
        self.sarIDlab.resize(50*self.scl,20*self.scl)  #A
        self.sarIDlab.setFont(fntSmall)
        self.sarIDlab.setText("SAR ID")
        self.sarOP = QtWidgets.QLineEdit(self.centralwidget)
        self.sarOP.move(420*self.scl, 775*self.scl)  #B
        self.sarOP.resize(50*self.scl,20*self.scl)  #B
        self.sarOP.setFont(fntSmall)
        self.sarOP.setText("1")   # set default OP period
        self.sarOPlab = QtWidgets.QLabel(self.centralwidget)
        self.sarOPlab.move(420*self.scl, 750*self.scl)  #B
        self.sarOPlab.resize(70*self.scl,20*self.scl)  #B
        self.sarOPlab.setFont(fntSmall)
        self.sarOPlab.setText("OPperiod")        
        self.prevOP = "1"
        self.t1 = QtWidgets.QLineEdit(self.centralwidget)
        self.t1.move(610*self.scl, 840*self.scl)  #B
        self.t1.resize(30*self.scl,20*self.scl)  #B
        self.t1.setFont(fntSmall)
        self.t2 = QtWidgets.QLineEdit(self.centralwidget)
        self.t2.move(650*self.scl, 840*self.scl)  #B
        self.t2.resize(30*self.scl,20*self.scl)  #B
        self.t2.setFont(fntSmall)

        
        #
        self.menubar = QtWidgets.QMenuBar(MainWindow)  ## ??  using 
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButton_prntScrn.setText(_translate("MainWindow", "PRNT SCRN"))
        self.pushButton_prnt104.setText(_translate("MainWindow", "PRNT 104"))
        self.setKeys(self.keyBrd)

      
    def dragMoveEvent(self, e):  
        e.accept(QtCore.QRect(0,0,1600,900))  ##QRect for moves
        #A e.accept(QtCore.QRect(0,0,3000,1800))  ##QRect for moves
...
class tabinfo(object):  ## table operations: tabload and tabmove
    def __init__(self):
        ...
        self.chk_file = ""
        ##### SAVE FILE 
        #### not used? self.SAVE_FILE_PATH = "DATA\saveSEARCH"   ## changing save to JSON
        savelen = 5
        self.saveSet = 0
        ...
        self.AssignCheck = [] ## [changed (0 - none, +1 to Assign, -1 to Unassigned) : Prev Value]
    ...
    def time_chk(self):  ## possibly only run/restart when new user add by _addSrchr
        ...
        try:
          ...
          if (self.masterBlink > 0):                     ## only starts timer when needed
            ...
            QtCore.QTimer.singleShot(500, lambda: self.time_chk())  ## 500 msec


    def srchr_chk(self, yy):  ## possibly only run/restart when new user add by _addSrchr
            ##   FOR remote user entry
        print("Top of srchr_chk")
        try:  # normal operation  
          if (True):  # indent fixer
              #Instead of doing IF here do it in rmtInProcess
              #                   to allow status change and possibly do updates
...
### DO we want to check for updated sign in file here OR
###    Just call process to update and let it check?

            '''
          server = "c:\\Tcard\\signin_files\\"     #Ax update for comp2
          ...
          #     if so, call routine to process new information
            ...
            '''
            if (DEBUG == 1): print("call to rmtInProcess")
            self.TApntr.rmtInProcess()     # if RMTchange != 1 get an immediate return when in SQL mode
        finally:                           # done when the above is complete  
          QtCore.QTimer.singleShot(15000, lambda: self.srchr_chk(yy))  ## 15 sec, use lambda to pass argument
          ##Ax QtCore.QTimer.singleShot(30000, lambda: self.srchr_chk(yy))  ## 30 sec, use lambda to pass argument
...
## CLEAR entire table first        
  ...
  ## count number of searchers in assigned area (not including column 0) and unassigned
        icntASSGN = 0
        icntUNASSGN = 0  
        for im in self.SRCHR:
            if (im[0] == 'END'): break    
            if (im[3] > 0 and im[3] < zz.Nunas_col):
                icntASSGN = icntASSGN + 1
            elif (im[3] >= zz.Nunas_col):
                icntUNASSGN = icntUNASSGN + 1
        xxx.tableWidget.setItem(0, zz.Nunas_col-1, QtWidgets.QTableWidgetItem("Count = "+str(icntASSGN)))
        xxx.tableWidget.setItem(0, zz.Nunas_col, QtWidgets.QTableWidgetItem("  Count = "+str(icntUNASSGN)))                       
        xxx.tableWidget.item(0, zz.Nunas_col-1).setFont(fntSmall)
        xxx.tableWidget.item(0, zz.Nunas_col).setFont(fntSmall)
...
#### START TEAMS LOOP:
   ...
   ##  Change sections to put entries into (Overhead, Teams-Assigned, UnAssigned:tflag0,1,2))
          ...
          #Cprint("TeamCheck: %s: %i: %i"%(self.TEAMS[self.lin][0],self.TEAMS[self.lin][1],self.TEAMS[self.lin][2]))
          fnt = fntBold                 ## for team headers
          color = QtCore.Qt.blue  ##A
          ##  add stuff to painter list - rounded rectangles
          if (colx != Ui_MainWindow.Nunas_col or rowy != 1): ## Don't place UnAssigned header
              ...
              if (colx > 0):
                xxx.tableWidget.setItem(rowy, colx, QtWidgets.QTableWidgetItem(text[0:15]+" "+ttype[0:3]+" "+tloc[0:4]))
              else:
...
#### START SRCHR LOOP:
        ...
        text1 = re.split(', | |,',text)  ## name is expected to have 2 words: last, first
        fnt = fntSmall2
        ...
        while (text != "END"):
            ...
            xxx.tableWidget.setItem(rowy, colx, QtWidgets.QTableWidgetItem(f"{text1[0][:13]:<13}"+" "+self.SRCHR[self.lin][1]+'\n  '+text1[1]))
            xxx.tableWidget.item(rowy, colx).setFont(fnt)
 #####  Blinking          
 ...
 #####  Find - change background    FIND
            if (DEBUG): print("At FIND background %s %s %s %s"%(self.SRCHR[self.lin][2],self.SRCHR[self.lin][1], \
                                                               self.findResource,self.findName) )    
        ...
        if (fromWhat == 0 and self.READIN == 1):    ## don't do when = 1, is blinker AND until READMEMB is active 
          ...
          with open("DATA\saveAll"+setName+".json", 'w') as outfile:  ## opens, saves, closes
            json.dump([self.TEAMS, self.SRCHR, self.MEMBERS, self.UNAS_USED, self.TEAM_NUM, \
                       self.RemoteSignInMode, self.eventID, self.AssignCheck], outfile) ## save TEAMS, SRCHR, MEMBERS and TEAM_NUM, Remote info
...
##@# check for space (for unassigned area use UNAS_USED  ENOUGH ROOM
        ...
        if (fnd_from_t != -1):     # from is pointing to team; note team ptr is calc_from_t for srchr movement
          ...
          nAdd = 1 #2                                  #Ax3   # srchr: 1 member + possible team header (why team header?)
        ...
        if (rowx > (self.Nrows - nAdd -1) or rowt == -1): #Ax3 # check for enough room (Okay for >=6 when not creating team)
          ...
          if (colt >= ww.Nunas_col and colt < ww.Nsets-1):          ## unassigned area
            ...
            for ixx in range(istrt,len(zz.UNAS_USED)):              #  search for next used entry in this column
              if (zz.UNAS_USED[ixx] == 1):                         
                break
              ifnd = ifnd + 1            # increment cause it is an available loc (note first check will do limit
                                         #    to just this column)
...
####  do we search for next team to find available room?
                    ...
                    if (colt == 0):       ## do not create team in col 0 (have to move to existing entry in col0)
                                             ##  either heading or member
                    ...
                    ## CREATE team        ((( if from an existing team, need to collapse the from-team )))
                      ...
                      if (self.sarOP.text() != self.prevOP):  ## reset team numb if op period is updated
                          zz.TEAM_NUM = 1
                          self.prevOP = self.sarOP.text()
                      typex = "GND"   # default
                      if (colf == ww.Nsets-1):
                        typex = zz.TEAMS[calc_from_t][4]   # came from Groups, so use type
                      tnum = str(zz.TEAM_NUM).zfill(2)  
                      zz.TEAMS.insert(zz.TEAMS.index(["END"]),["T"+self.sarOP.text()+tnum, colt, rowt, 1, typex, "IC", 0.0]) # insert prior to END
